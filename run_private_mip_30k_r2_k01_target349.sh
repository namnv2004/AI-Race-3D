...
for scene in $SCENES; do
    ...
    if [[ -f "$checkpoint" ]] && python scripts/verify_submission.py --split-root data/round1/phase1/private_set1 --submission-dir "$SOURCE_OUTPUT_DIR" --scenes "$scene" > "$scene_verify_log" 2>&1; then
...
python scripts/recompress_submission_jpegs.py \
...
echo "Sending final Mip zip to ironcat at $(date -Is)"
tailscale file cp --verbose "$ZIP_PATH" ironcat: > reports/experiments/log_private_mip_target349_taildrop.txt 2>&1
